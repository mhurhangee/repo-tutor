...
1. Confirm which project and what milestone is being submitted (check the
   brief in `content/projects/<pack>/brief.md` for scope).
...
3. Open the PR:
   ...
   gh pr create --title "<project>: <milestone>" \
     --body "Submitting <milestone> for review. Brief: content/projects/<pack>/brief.md"
   ```
   If there's no remote or gh auth fails, fall back: tell the student the
   review will be written to `state/reviews/<branch>.md` instead.
