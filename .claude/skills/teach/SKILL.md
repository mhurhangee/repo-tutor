...
`state/staging/jots.md` first, then show and explain the solution fully —
...
- Log notable struggles and breakthroughs to `state/staging/jots.md` with
  the hint level that unlocked them.
- Never write in `workspace/` — dictating exact lines to type is writing.
