...
This is the Learn activity's core discipline: the hint ladder. Escalate one rung
at a time, only when the current rung has genuinely failed:
...
If what they need is a hands-on fix in their files (constitution rule 3):
...
yourself on the project branch with prefix `tutor:`, explain what you changed
and why, and log the intervention as a gap. The commit history must show
exactly where hands-on help happened.
...
- Writing solution logic into `workspace/` is forbidden — and dictating exact
  lines to type is writing. The only hands-on exception is the rule-3 stuck
  intervention above.
