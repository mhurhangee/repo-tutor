...
Read `state/topics.yaml` and `state/syllabus.md` (plus ledger files for
...
Write to `content/projects/<name>/`:

- `brief.md` — the negotiated goal, milestones, which concepts it
  exercises, and what "done" means per milestone. Written to the student.
- `scaffold/` — minimal starting files: entry point stubs, empty modules
...
## 4. Student instantiates

The student copies the scaffold and creates their branch — not you.
Walk them through running:

```bash
cp -r content/projects/<name>/scaffold workspace/projects/<name>
...
Append to `state/staging/jots.md`: project name, target topics, negotiated
